...
TRAIN_DATA_FILE = os.path.join(FILE_BASE, "df_train_sst.pickle")
TEST_DATA_FILE = os.path.join(FILE_BASE, "df_test_sst.pickle")

TRAIN_SIZE = 100
TEST_SIZE = 10

HIDDEN_VARS=['Time', 'UA', 'UA_mod']
...
def gen_train_spec():
    ...
    fault_list = []
    for i in range(1, 5):
        fault_list += loader.get_fault_list(1,  {'fouling_fault': 0.0  }, context='bc == %d' % i)
        for min_start in range(0, 672, 168):
            start_context = 'f1_start > %d & f1_start <= %d' % (min_start, min_start + 168)
            fault_list += loader.get_fault_list(6,  {'fouling_fault': 1.0  }, context='bc == %d & %s' % (i, start_context))
...
def gen_test_spec():
    ...
    fault_list += loader.get_fault_list(1,  {'fouling_fault': 0.0  }, context='bc == 0')
    fault_list += loader.get_fault_list(6,  {'fouling_fault': 1.0  }, context='bc == 0 & f1_final <= 0.5')
    fault_list += loader.get_fault_list(3,  {'fouling_fault': 1.0  }, context='bc == 0 & f1_final > 0.5')
