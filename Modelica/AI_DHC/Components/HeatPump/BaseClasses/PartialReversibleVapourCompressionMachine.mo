within AI_DHC.Components.HeatPump.BaseClasses;
partial model PartialReversibleVapourCompressionMachine "Grey-box model for reversible heat pumps and chillers using a black-box to simulate the refrigeration cycle"
...
//General
  ...
  replaceable PartialInnerCycle                          innerCycle constrainedby AixLib.Fluid.BaseClasses.PartialInnerCycle
  ...
  parameter Modelica.Units.SI.Power Q_useNominal(start=0) "Nominal usable heat flow of the vapour compression machine (HP: Heating; Chiller: Cooling)"
  ...
  parameter Modelica.Units.SI.Frequency refIneFre_constant "Cut off frequency for inertia of refrigerant cycle"
...
//Condenser
  parameter Modelica.Units.SI.MassFlowRate mFlow_conNominal "Manual input of the nominal mass flow rate (if not automatically calculated)"
    annotation (Dialog(
      group="Parameters",
      tab="Condenser",
      enable=not use_autoCalc), Evaluate=true);
  parameter Modelica.Units.SI.Volume VCon "Manual input of the condenser volume (if not automatically calculated)"
    annotation (Evaluate=true, Dialog(
      group="Parameters",
      tab="Condenser",
      enable=not use_autoCalc));
  parameter Modelica.Units.SI.PressureDifference dpCon_nominal "Pressure drop at nominal mass flow rate"
  ...
  parameter Modelica.Units.SI.ThermalConductance GConIns
    ...
    annotation (Evaluate=true, Dialog(
      ...
      enable=use_conCap));
//Evaporator
  parameter Modelica.Units.SI.MassFlowRate mFlow_evaNominal "Manual input of the nominal mass flow rate (if not automatically calculated)"
    annotation (Dialog(
      group="Parameters",
      tab="Evaporator",
      enable=not use_autoCalc), Evaluate=true);
  parameter Modelica.Units.SI.Volume VEva "Manual input of the evaporator volume (if not automatically calculated)"
    annotation (Evaluate=true, Dialog(
      group="Parameters",
      tab="Evaporator",
      enable=not use_autoCalc));
  parameter Modelica.Units.SI.PressureDifference dpEva_nominal "Pressure drop at nominal mass flow rate"
  ...
  parameter Modelica.Units.SI.ThermalConductance GEvaIns
    ...
    annotation (Evaluate=true, Dialog(
      ...
      enable=use_evaCap));
//Assumptions
  parameter Modelica.Units.SI.Time tauSenT=1 "Time constant at nominal flow rate (use tau=0 for steady-state sensor, but see user guide for potential problems)"
  ...
  parameter Modelica.Units.SI.Time tauHeaTraEva=1200 "Time constant for heat transfer in temperature sensors in evaporator, default 20 minutes"
    annotation (Dialog(
      tab="Assumptions",
      group="Temperature sensors",
      enable=transferHeat), Evaluate=true);
  parameter Modelica.Units.SI.Temperature TAmbEva_nominal=273.15 "Fixed ambient temperature for heat transfer of sensors at the evaporator side"
    annotation (Dialog(
      tab="Assumptions",
      group="Temperature sensors",
      enable=transferHeat));
  parameter Modelica.Units.SI.Time tauHeaTraCon=1200 "Time constant for heat transfer in temperature sensors in condenser, default 20 minutes"
    annotation (Dialog(
      tab="Assumptions",
      group="Temperature sensors",
      enable=transferHeat), Evaluate=true);
  parameter Modelica.Units.SI.Temperature TAmbCon_nominal=291.15 "Fixed ambient temperature for heat transfer of sensors at the condenser side"
    annotation (Dialog(
      tab="Assumptions",
      group="Temperature sensors",
      enable=transferHeat));
...
//Initialization
  ...
  parameter Modelica.Media.Interfaces.Types.Temperature TCon_start=Medium_con.T_default
    ...
    annotation (Evaluate=true,Dialog(tab="Initialization", group="Condenser"));
  parameter Modelica.Units.SI.Temperature TConCap_start=Medium_con.T_default "Initial temperature of heat capacity of condenser"
    annotation (Dialog(
      tab="Initialization",
      group="Condenser",
      enable=use_conCap));
  ...
  parameter Modelica.Media.Interfaces.Types.Temperature TEva_start=Medium_eva.T_default
    "Start value of temperature"
    annotation (Evaluate=true,Dialog(tab="Initialization", group="Evaporator"));
  parameter Modelica.Units.SI.Temperature TEvaCap_start=Medium_eva.T_default "Initial temperature of heat capacity at evaporator"
    annotation (Dialog(
      tab="Initialization",
      group="Evaporator",
      enable=use_evaCap));
...
//Advanced
  ...
  Modelica.Blocks.Interfaces.RealInput nSet if not useBusConnectorOnly "Input signal speed for compressor relative between 0 and 1"
                                                                 annotation (Placement(
        transformation(extent={{-132,4},{-100,36}})));
  BaseClasses.VapourCompressionMachineControlBus sigBus
  ...
  AixLib.Fluid.Sensors.TemperatureTwoPort senT_a2(
    ...
    final tau=tauSenT,
    final m_flow_nominal=mFlow_evaNominal_final*scalingFactor) "Temperature at sink inlet"
    annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=0,
        origin={38,-86})));
  AixLib.Fluid.Sensors.TemperatureTwoPort senT_b2(
    ...
    final tauHeaTra=tauHeaTraEva,
    final tau=tauSenT,
    final m_flow_nominal=mFlow_evaNominal_final*scalingFactor) "Temperature at sink outlet"
    annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=0,
        origin={-52,-86})));
  AixLib.Fluid.Sensors.MassFlowRate mFlow_eva(redeclare final package Medium = Medium_eva, final allowFlowReversal=allowFlowReversalEva)
  ...
  AixLib.Fluid.Sensors.TemperatureTwoPort senT_b1(
    ...
    final m_flow_nominal=mFlow_conNominal_final*scalingFactor) "Temperature at sink outlet"
    annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=0,
        origin={38,92})));
  AixLib.Fluid.Sensors.TemperatureTwoPort senT_a1(
    ...
    final tau=tauSenT,
    final m_flow_nominal=mFlow_conNominal_final*scalingFactor,
    final tauHeaTra=tauHeaTraCon) "Temperature at sink inlet"
    annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=0,
        origin={-34,90})));
  AixLib.Fluid.Sensors.MassFlowRate mFlow_con(final allowFlowReversal=allowFlowReversalEva, redeclare final package Medium = Medium_con)
    "Mass flow sensor at the evaporator" annotation (Placement(transformation(
        origin={-76,60},
        extent={{-10,10},{10,-10}},
        rotation=0)));
...
protected
  ...
  parameter Modelica.Units.SI.MassFlowRate autoCalc_mFlow_eva=if machineType then max(0.00004*Q_useNominal - 0.3177, autoCalc_mFlow_min) else max(0.00005*
      Q_useNominal - 0.5662, autoCalc_mFlow_min);
  parameter Modelica.Units.SI.MassFlowRate autoCalc_mFlow_con=if machineType then max(0.00004*Q_useNominal - 0.6162, autoCalc_mFlow_min) else max(0.00005*
  ...
  parameter Modelica.Units.SI.Volume autoCalc_VEva=if machineType then max(0.0000001*Q_useNominal - 0.0075, autoCalc_Vmin) else max(0.0000001*Q_useNominal - 0.0066,
      autoCalc_Vmin);
  parameter Modelica.Units.SI.Volume autoCalc_VCon=if machineType then max(0.0000001*Q_useNominal - 0.0094, autoCalc_Vmin) else max(0.0000002*Q_useNominal - 0.0084,
      autoCalc_Vmin);
