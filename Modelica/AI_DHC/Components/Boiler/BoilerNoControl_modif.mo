...
model BoilerNoControl_modif "Boiler model with physics only"
  ...
  parameter Modelica.Units.SI.ThermalConductance G=0.005*Q_nom/40 "Constant thermal conductance to environment(G=Q_loss/dT)";
  parameter Modelica.Units.SI.HeatCapacity C=1.5*Q_nom "Heat capacity of metal (J/K)";
  parameter Modelica.Units.SI.Volume V=paramBoiler.volume "Volume";

  parameter Modelica.Units.SI.Power Q_nom=paramBoiler.Q_nom "Nominal heating power";
  ...
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor internalCapacity(
    ...
    annotation (Placement(transformation(
        ...
        rotation=90,
        origin={4,-50})));
  BaseClass.ThermalConductor                                ConductanceToEnv
  ...
  Modelica.Blocks.Sources.RealExpression NominalGasConsumption(final y=Q_nom/
        max(etaLoadBased[:,2]*max(etaTempBased[:,2]))) "Nominal gas power"
  ...
  Modelica.Blocks.Tables.CombiTable1Dv efficiencyTableLoadDepending(
    ...
    final columns={2},
    final smoothness=Modelica.Blocks.Types.Smoothness.ContinuousDerivative) "Table with efficiency parameters"
  ...
  Modelica.Blocks.Math.Product etaCalculation
    "calculates the efficiency of the boiler" annotation (Placement(
        ...
        rotation=0,
        origin={10,50})));
  Modelica.Blocks.Tables.CombiTable1Dv efficiencyTableTdepending(
    final tableOnFile=false,
    final table=etaTempBased,
    final columns={2},
    final smoothness=Modelica.Blocks.Types.Smoothness.ContinuousDerivative) "Table with efficiency parameters"
    annotation (Placement(transformation(extent={{-60,20},{-39,41}})));
  Modelica.Blocks.Interfaces.RealInput G_ext_modifier "0 to 1" annotation (Placement(transformation(
        extent={{-20,-20},{20,20}},
        rotation=270,
        origin={66,120}), iconTransformation(
        extent={{-4.61529,35.3846},{15.3843,15.3846}},
        rotation=270,
        origin={-15.3846,115.385})));
  Modelica.Blocks.Sources.RealExpression G_modifier(final y=G*(1 + G_ext_modifier)) "Nominal gas power"
    annotation (Placement(transformation(extent={{54,-44},{26,-28}})));
  Modelica.Blocks.Sources.RealExpression etaT_modifier(final y=(1 - etaT_ext_modifier)*efficiencyTableTdepending.y[1]) "Modifier of efficiency"
    annotation (Placement(transformation(extent={{16,14},{-12,30}})));
  Modelica.Blocks.Interfaces.RealInput etaT_ext_modifier annotation (Placement(transformation(
        extent={{-20,-20},{20,20}},
        rotation=270,
        ...
        rotation=270,
        origin={-12,110})));
  Modelica.Blocks.Interfaces.RealOutput eta_tot "Connector of Real output signal" annotation (Placement(transformation(extent={{100,36},{120,56}})));
equation
  connect(vol.heatPort, ConductanceToEnv.port_a)
    annotation (Line(points={{-50,-70},{-50,-24},{-40,-24}}, color={191,0,0}));
  connect(vol.heatPort, internalCapacity.port)
    annotation (Line(points={{-50,-70},{-50,-36},{-44,-36},{-44,-50},{-6,-50}},
                                                            color={191,0,0}));
  ...
  connect(ConductanceToEnv.port_b, T_amb)
    annotation (Line(points={{-20,-24},{10,-24},{10,-20},{40,-20}},
                                                  color={191,0,0}));
  ...
  connect(senTCold.T, T_in) annotation (Line(points={{-70,-69},{-70,-64},{-74,-64},{-74,-12},{-44,-12},{-44,-10},{26,-10},{26,-6},{84,-6},{84,26},{110,26}},
                     color={0,0,127}));
  ...
  connect(efficiencyTableLoadDepending.y[1], etaCalculation.u1) annotation (
      Line(points={{-17.95,60.5},{-10.975,60.5},{-10.975,56},{-2,56}}, color={0,
          0,127}));
  connect(senTCold.T, efficiencyTableTdepending.u[1]) annotation (Line(points={{-70,-69},{-70,-64},{-74,-64},{-74,30.5},{-62.1,30.5}}, color={0,0,127}));
  connect(QflowCalculation.u1, etaCalculation.y)
    annotation (Line(points={{-54,14},{21,14},{21,50}}, color={0,0,127}));
  connect(G_modifier.y, ConductanceToEnv.G_ext) annotation (Line(points={{24.6,-36},{10,-36},{10,-34},{-2,-34},{-2,-30.6},{-20,-30.6}}, color={0,0,127}));
  connect(etaT_modifier.y, etaCalculation.u2) annotation (Line(points={{-13.4,22},{-20,22},{-20,44},{-2,44}}, color={0,0,127}));
  connect(etaCalculation.y, eta_tot) annotation (Line(points={{21,50},{94,50},{94,46},{110,46}}, color={0,0,127}));
  annotation (
