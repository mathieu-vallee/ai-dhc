...
model Stratified_modified
  "Model of a stratified tank for thermal energy storage"
  extends Components.Storage.PartialStratified_modified(vol(each nPorts=3));
  ...
  Modelica.Blocks.Interfaces.RealOutput Pdecharge
    "Puissance positive lorsque le stockage se vide (en W)"
    annotation (Placement(transformation(extent={{102,-100},{122,-80}})));
  Modelica.Blocks.Sources.RealExpression FluxEnthalHaut(y=-port_b.m_flow*
        actualStream(port_b.h_outflow))
    "Flux Enthalpique sortant en haut - (> 0 si sortant du stockage)"
    annotation (Placement(transformation(extent={{-48,-80},{-28,-60}})));
  Modelica.Blocks.Sources.RealExpression FluxEnthalBas(y=port_a.m_flow*
...
equation
  ...
  connect(FluxEnthalHaut.y, add.u1) annotation (Line(points={{-27,-70},{8,
          -70},{8,-84},{36,-84}}, color={0,0,127}));
  connect(FluxEnthalBas.y, add.u2) annotation (Line(points={{-25,-88},{28,
          -88},{28,-96},{36,-96}}, color={0,0,127}));
  connect(add.y, Pdecharge) annotation (Line(points={{59,-90},{112,-90}}, color={0,0,127}));
  annotation (
