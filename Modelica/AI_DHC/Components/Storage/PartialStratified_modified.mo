...
model PartialStratified_modified
  ...
  Modelica.Units.SI.ThermalConductivity kIns_Modifier=kIns*(1 +
      kIns_Ext_Modifier) "Specific heat conductivity of insulation";
  ...
  Modelica.Units.SI.ThermalConductance conTopSeg=ATan*kIns_Modifier/dIns
  ...
  BaseClass.ThermalConductor[nSeg] conWal "Thermal conductance through tank wall"
    annotation (Placement(transformation(extent={{10,36},{20,48}})));
  BaseClass.ThermalConductor                              conTop
                       "Thermal conductance through tank top"
    annotation (Placement(transformation(extent={{10,54},{20,66}})));
  BaseClass.ThermalConductor conBot "Thermal conductance through tank bottom"
  ...
  Modelica.Blocks.Interfaces.RealInput kIns_Ext_Modifier "0 to 1"
    annotation (Placement(transformation(extent={{-124,-78},{-84,-38}})));
  Modelica.Blocks.Sources.RealExpression G_conTop(y=ATan*kIns_Modifier/dIns)
    annotation (Placement(transformation(extent={{-98,36},{-78,56}})));
  Modelica.Blocks.Sources.RealExpression G_conWal(y=2*Modelica.Constants.pi*
        kIns_Modifier*hSeg/Modelica.Math.log((rTan + dIns)/rTan))
...
//   conBot.G =conTopSeg;
  ...
  connect(buo.heatPort, vol.heatPort)    annotation (Line(
      points={{-22,78},{-6,78},{-6,60},{6,60},{6,-6}},
      color={191,0,0}));
  ...
  connect(vol[1].heatPort, conTop.port_a)    annotation (Line(points={{6,-6},{
          6,52},{-8,52},{-8,60},{10,60}},    color={191,0,0}));
  connect(vol.heatPort, conWal.port_a)    annotation (Line(points={{6,-6},{6,
          42},{10,42}},                  color={191,0,0}));
  ...
  connect(conWal.port_b, heaFloSid.port_a)
    annotation (Line(points={{20,42},{26,42},{26,40},{30,40}},
                                               color={191,0,0}));
  ...
  connect(theCol.port_b, heaPorSid) annotation (Line(
      ...
      color={191,0,0},
      smooth=Smooth.None));
  connect(G_conTop.y, conTop.G_ext) annotation (Line(points={{-77,46},{8,46},
          {8,52},{24,52},{24,63.96},{20,63.96}}, color={0,0,127}));
  connect(G_conTop.y, conBot.G_ext) annotation (Line(points={{-77,46},{8,46},
          {8,52},{24,52},{24,23.96},{20,23.96}}, color={0,0,127}));
  connect(G_conWal.y, replicator.u)
    annotation (Line(points={{-45,30},{-32,30}}, color={0,0,127}));
  connect(replicator.y, conWal.G_ext) annotation (Line(points={{-9,30},{22,
          30},{22,42},{20,42},{20,45.96}}, color={0,0,127}));
  annotation (
...
Icon(graphics={
        ...
        Line(
          ...
          color={127,0,0},
          pattern=LinePattern.Dot)}));
end PartialStratified_modified;
